...
|---|---|---|
| `number.*_max_service_current` | 1–200 A | Your service current limit (breaker rating). The integration will never allow total consumption to exceed this. Changing it takes effect on the next power-meter event — no reload needed. You can raise it temporarily to accommodate a higher-power session or lower it to reserve more headroom for other loads. |
