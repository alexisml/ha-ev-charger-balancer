...
Author: alexisml
Status: draft
Summary: Phased implementation roadmap for the EV charger load-balancing integration — Phase 1 is the MVP (single-charger), Phase 2 adds multi-charger support.

---

This document contains the implementation roadmap.  It was split out from [`docs/development-memories/2026-02-19-research-plan.md`](../../development-memories/2026-02-19-research-plan.md) so the delivery phases are clearly separated.
...
| PR-5-MVP: Manual override + observability | Expose `ev_lb.set_limit` service and add/verify diagnostic state updates needed for troubleshooting. | Manual override changes runtime limits safely and state reflects changes without restart; unit tests cover limit and state transitions. |
| PR-6-MVP: Test stabilization + HACS release readiness | Finalize HACS requirements (`manifest.json`, `hacs.json`, repository structure/docs), complete integration tests (`pytest-homeassistant-custom-component`), and prepare first release. | CI is green on 3 consecutive runs, installation via HACS works, and docs cover configuration + troubleshooting. |

## Phase 2 — Multi-charger support (post-MVP)

Phase 2 begins after the MVP is released and stable.  It adds the water-filling fair distribution algorithm and a runtime options flow to manage multiple chargers.
...
| Manual override + observability | PR-5-MVP | alexisml | +9 days | `ev_lb.set_limit` service + diagnostic state | |
| Test stabilization + HACS release | PR-6-MVP | alexisml | +11 days | Full integration tests, HACS-ready release | |

### Phase 2 — Multi-charger support

| Step | PR | Owner | ETA | Deliverable | Status |
|------|-----|-------|-----|-------------|--------|
| Multi-charger fairness | PR-5 | alexisml | post-MVP | `distribute_current` water-filling logic | |
| Runtime charger management | PR-6 | alexisml | post-MVP | Options flow for add/remove chargers | |
